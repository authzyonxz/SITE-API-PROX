...
export async function updateUserDeviceId(userId, deviceId) {
    const db = await getDb();
    if (!db)
        throw new Error("Database not available");
...
export async function updateUserMaxIps(userId, maxDevices) {
    const db = await getDb();
    if (!db)
        throw new Error("Database not available");
    await db.update(localUsers).set({ maxDevices }).where(eq(localUsers.id, userId));
}
export async function resetUserSession(userId) {
    const db = await getDb();
    if (!db)
        throw new Error("Database not available");
...
// ─── Generated keys ───────────────────────────────────────────────────────────
export async function saveGeneratedKey(data) {
    const db = await getDb();
    if (!db)
        throw new Error("Database not available");
    await db.insert(generatedKeys).values(data);
}
export async function markKeyDeleted(keyValue) {
    const db = await getDb();
    if (!db)
        throw new Error("Database not available");
...
export async function getKeyStats() {
    const db = await getDb();
    if (!db)
        return { active: 0, expired: 0, deleted: 0 };
    const now = new Date();
    // Update expired keys
    await db.update(generatedKeys)
        .set({ status: "expired" })
...
export async function getKeysByUser(userId) {
    const db = await getDb();
    if (!db)
        return [];
...
// ─── Key Management ───────────────────────────────────────────────────────────
export async function getKeysByUserId(userId) {
    const db = await getDb();
    if (!db)
        return [];
    return db.select().from(generatedKeys).where(eq(generatedKeys.createdById, userId));
}
export async function findKeyCreator(keyValue) {
    const db = await getDb();
    if (!db)
        return null;
...
export async function deleteKeysByUserId(userId) {
    const db = await getDb();
    if (!db)
        throw new Error("Database not available");
    await db.update(generatedKeys)
...
export async function updateProxyStatus(id, status) {
    const db = await getDb();
    if (!db)
        throw new Error("Database not available");
...
export async function isIpBlacklisted(ipAddress) {
    const db = await getDb();
    if (!db)
        return false;
...
export async function countKeysGeneratedRecently(userId, minutes) {
    const db = await getDb();
    if (!db)
        return 0;
